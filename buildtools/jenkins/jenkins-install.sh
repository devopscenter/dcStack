...
if ! (crontab -u jenkins -l |grep '^[^#].*jenkins-backup.sh\b.*'); then
    (crontab -u jenkins -l 2>/dev/null; echo "11 03  *   *   *     /home/ubuntu/dcStack/buildtools/jenkins/jenkins-backup.sh ${theHostName}") | crontab -u jenkins -
fi
set +x 
